...
(define (->0 n)
  (symbol->string n))
(define (->1 n m)
  (format #f "~a(~a)" n (car m)))
(define (->2 n m)
  (format #f "~a(~a,~a)" n (car m) (cadr m)))
(define (->n n m)
  (format #f "~a(~{'~a'~^,~})" n m))
...
(define (->mysql-type name . args)
  (case name
    ...
    ((char) (->1 name args))
    ((tinytext) (->0 name)) ; TINYTEXT Holds a string with a maximum length of 255 characters
    ((text) (->0 name)) ; TEXT Holds a string with a maximum length of 65,535 characters
    ((blob) (->0 name)) ; BLOB For BLOBs (Binary Large OBjects). Holds up to 65,535 bytes of data
    ((mediumtext) (->0 name)) ; MEDIUMTEXT Holds a string with a maximum length of 16,777,215 characters
    ((mediumblob) (->0 name)) ; MEDIUMBLOB For BLOBs (Binary Large OBjects). Holds up to 16,777,215 bytes of data
    ((longtext) (->0 name)) ; LONGTEXT Holds a string with a maximum length of 4,294,967,295 characters
    ((longblob) (->0 name)) ; LONGBLOB For BLOBs (Binary Large OBjects). Holds up to 4,294,967,295 bytes of data
...
(define (->postgresql-type name . args)
  (case name
    ;; Numeric Types
    ((serial) (->0 name)) ; serial 4 bytes autoincrementing integer 1 to 2147483647
    ((bigserial) (->0 name)) ;  bigserial 8 bytes large autoincrementing integer 1 to 9223372036854775807
    ((int) (->0 name))

    ;; Character Types
    
    ;; char for 1 byte single-byte internal type
    ((char) (if (null? args) (->0 name) (->1 name args)))
    ((name) (->0 name)) ; 64 bytes internal type for object names
...
(define (->sql-general-type name . args)
  (case name
    ...
    ((char-var) (->1 "character varying" args)) ; CHARACTER VARYING(n) Character string. Variable length. Maximum length n
    ((binary) (->1 name args)) ; BINARY(n) Binary string. Fixed-length n
    ((boolean) (->0 name)) ; BOOLEAN Stores TRUE or FALSE values
    ((varbinary) (->1 name args)) ; VARBINARY(n)
    ((binary-var) (->1 "binary varying" args)) ; Binary VARYING(n) Binary string. Variable length. Maximum length n
    ((smallint) (->0 name)) ; SMALLINT Integer numerical (no decimal). Precision 5
    ;; INTEGER(p) Integer numerical (no decimal). Precision p
    ;; or
    ;; INTEGER Integer numerical (no decimal). Precision 10
    ((integer) (if (null? args) (->0 name) (->1 name args)))
    ((bigint) (->0 name)) ; BIGINT Integer numerical (no decimal). Precision 19
    ...
    ((float) (if (null? args) (->0 name) (->1 name args)))
    ((real) (->0 name)) ; REAL Approximate numerical, mantissa precision 7
    ((double-precision) (->0 name)) ; DOUBLE PRECISION Approximate numerical, mantissa precision 16
    ((date) (->0 name)) ; DATE Stores year, month, and day values
    ((time) (->0 name)) ; TIME Stores hour, minute, and second values
    ((timestamp) (->0 name)) ; TIMESTAMP Stores year, month, day, hour, minute, and second values
    ;; INTERVAL Composed of a number of integer fields, representing a period of time, depending on the type of interval
    ((interval) (->0 name))
    ((array) (->0 name)) ; ARRAY A set-length and ordered collection of elements
    ((multiset) (->0 name)) ; MULTISET A variable-length and unordered collection of elements
    ((xml) (->0 name)) ; XML Stores XML data
    (else #f)))
