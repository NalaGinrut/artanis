...
(define (make-sm-string-template template)
  ...
  (define (parse-it name)
    (let ((nn (sm-name-parser name)))
      (match nn
        (`(name ,n) (names-table-add! n 'name))
        (`(post-qstr ,pn) (names-table-add! pn 'post-qstr))
  ...
  (let* ((rev-items (irregex-fold
    ...
    (lambda (rc . kargs)
      (define (item->string item)
        (if (string? item)
            ...
            (cond
             ...
             ((cdr item) (cdr item))
             (else (error "Missing keyword" (car item))))))
      (define post-data #f)
      ...
      (string-concatenate (append (map qstr-ref items) (map item->string items))))))
